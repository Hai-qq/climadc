...
`summary.csv` provides the same policy rows for simple review. The absolute `1e-8` tolerance is
smaller than the replay's `1e-7 kWh` feasibility tolerance and only accommodates cross-platform
floating-point solver noise; it is not used to alter or select results.
